...
#def get_events_data(metadata_keys, lat, lon, src_cod, dst_cod, path, event_type, test_type, time_start):
def get_events_data(metadata_keys, lat, lon, src, dst, src_cod, dst_cod, path, event_type, test_type, time_start):
    ...
    str_time_start = "?time-start=" + str(time_start)
    #str_limit = "&limit=" + str(sys.maxsize) # int max
    ...
    for metadata_key in metadata_keys:
        for event in metadata_keys[metadata_key]["events_base_uri"]:
            ...
            url = BASE+event+str_time_start+str_limit
...
#def main(src, dst, interface, test_id, path, event_type, test_type, time_start=1622505600): # time-start = 01/06/2021
def main(interface, test_id, path, event_type, test_type, time_start=1622516400): # time-start = 01/06/2021
    ...
    for src in pops:
        for dst in pops:
            ...
            get_events_data(hash_mk[src+dst], lat, lon, src, dst, src_cod, dst_cod, path, event_type, test_type, time_start)
...
if __name__ == "__main__":
    ...
    event_type = None
    try:
        opts, args = getopt.getopt(sys.argv[1:],None,["time-start=","test-type=","event-type="])
    ...
    time_start = int(datetime.datetime(year, month, day, 0, 0, 0).timestamp())
    ...
    main(interface, test_id, path, event_type, test_type, time_start)
